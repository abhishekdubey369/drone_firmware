...
TABS.motors.initialize = function (callback) {
    ...
    function process_html() {
        ...
        //unlock checkbox to be controlled by master
   ...
   checkedAttr = [];
    $('#category-list :checkbox').each(function(i, item){
        ...
        if($(item).is(':checked'))
        {
            ...
            checkedAttr.push(index); 
        }
        index++;
    });
   console.log("checkedAttr:", checkedAttr);
 //  var val = checkedAttr.val();
});
  ...
  /* var index = 0;
    $('#m').each(function(i,this) {
        if (this.checked) {
            console.log("Box is checked at index=" + index);
        }
        index++;
    });
...
*/
        ...
        $('div.tst input.master').on('input', function () {
            ...
            $('div.tst input:not(:disabled, :last)').val(val);
