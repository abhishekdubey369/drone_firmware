...
void annexCode(void)
{
    ...
    if (controlSwitch && ARMING_FLAG(ARMED)) {	// for localisation
...
void executePeriodicTasks(void)
{
...
#ifdef OPTIC_FLOW
    	// AdityaLakshmi
    	controlSwitch = runUserCode; 		//This variable decides if PosController should run
    	UpdateOpticFlow(currentTime);
    	PositionEstimate(currentTime);		// Also includes the position controller call

    	//PollOpticFlow(currentTime);
        //updateOpticFlow(currentTime);
    ...
    case UPDATE_LASER_TOF_TASK:
        ...
        getRange_L1(currentTime); //Polling at 50ms
...
void loop(void)
{
    ...
    if (shouldProcessRx(currentTime)) {
        ...
        executePeriodicTasks(); //Happens at roughly while(1)
    ...
    if (masterConfig.looptime == 0 || (int32_t)(currentTime - loopTime) >= 0) {
        ...
        annexCode();	//Happens at looptime based freq
...
#endif
// AdityaLakshmi
// Changes made and called on top
//#ifdef OPTIC_FLOW
       // selectVelOrPosmode();
//#endif
