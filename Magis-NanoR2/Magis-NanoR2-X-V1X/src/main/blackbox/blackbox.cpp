...
//#define DEBUG_AXISPID
#define DEBUG_LOG
#define DEBUG_LOG_COUNT 12
...
#ifdef DEBUG_LOG
	{"Vel X",  -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Vel Y", -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Des Vel X",       -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Des Vel Y",       -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Squal",       -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Shutter",		-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"LHeight",     -1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Roll",		-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Pitch",		-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"Yaw",			-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"EstAlt",		-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
	{"EstVelocity",	-1, SIGNED,   .Ipredict = PREDICT(0),       .Iencode = ENCODING(SIGNED_VB),   .Ppredict = PREDICT(0),      .Pencode = ENCODING(SIGNED_VB), CONDITION(ALWAYS)},
#endif
...
#ifdef DEBUG_LOG
    int16_t log_var[DEBUG_LOG_COUNT];
#endif
...
#ifdef DEBUG_LOG
    	blackboxWriteSigned16VBArray(blackboxCurrent->log_var, DEBUG_LOG_COUNT);
#endif
...
#ifdef DEBUG_LOG
    blackboxWriteSigned16VBArray(blackboxCurrent->log_var, DEBUG_LOG_COUNT);
    //blackboxWriteMainStateArrayUsingAveragePredictor(offsetof(blackboxMainState_t, log_var), DEBUG_LOG_COUNT);
#endif
...
static void loadMainState(void)
{
...
#endif
#ifdef DEBUG_LOG
    //Edit blackbox variables

    // Position control variables

    /*blackboxCurrent->log_var[0]=(int16_t)VelocityX;
    blackboxCurrent->log_var[1]=(int16_t)VelocityY;
    blackboxCurrent->log_var[2]=(int16_t)debugOpticFlowVar; // Estimated position X
    blackboxCurrent->log_var[3]=(int16_t)debugOpticFlowVar1; // Estimated position y
    blackboxCurrent->log_var[4]=(int16_t)debugOpticFlowVar2; // Desired vel X
    blackboxCurrent->log_var[5]=(int16_t)debugOpticFlowVar3; // Desired vel Y
    blackboxCurrent->log_var[6]=(int16_t)debugOpticFlowVar6; // Desired pos X
    blackboxCurrent->log_var[7]=(int16_t)debugOpticFlowVar4;  // surface_quality
    blackboxCurrent->log_var[8]=(int16_t)debugOpticFlowVar5; // shutter
    blackboxCurrent->log_var[9]=(int16_t)debugOpticFlowVar7; // Desired pos Y
    blackboxCurrent->log_var[10]=(int16_t)getEstAltitude();
    blackboxCurrent->log_var[11]=(int16_t)getEstVelocity();*/
	...
	blackboxCurrent->log_var[2]=(int16_t)debugOpticFlowVar;  // Estimated position X
	blackboxCurrent->log_var[3]=(int16_t)debugOpticFlowVar1; // Estimated position y
	blackboxCurrent->log_var[4]=(int16_t)debugOpticFlowVar8; // Optic flow velocity x
	blackboxCurrent->log_var[5]=(int16_t)debugOpticFlowVar9; // Optic flow velocity y
	blackboxCurrent->log_var[6]=(int16_t)debugOpticFlowVar10; // del_X_derotated
	blackboxCurrent->log_var[7]=(int16_t)debugOpticFlowVar4; //  surface quality
	blackboxCurrent->log_var[8]=(int16_t)debugOpticFlowVar5; // shutter
	blackboxCurrent->log_var[9]=(int16_t)debugOpticFlowVar11; // del_Y_derotated
	blackboxCurrent->log_var[10]=(int16_t)getEstAltitude();
	blackboxCurrent->log_var[11]=(int16_t)RangingMeasurementData_L1.RangeMilliMeter; // Laser Altitude
	*/
    ...
    blackboxCurrent->log_var[2]=(int16_t)debugOpticFlowVar;  // Estimated position X
    blackboxCurrent->log_var[3]=(int16_t)debugOpticFlowVar1; // Estimated position y
    blackboxCurrent->log_var[4]=(int16_t)debugAccelvar; //  Accel_X
    blackboxCurrent->log_var[5]=(int16_t)debugAccelvar1; // Accel_Y
    blackboxCurrent->log_var[6]=(int16_t)debugAccelvar2; // Accel_correction X
    blackboxCurrent->log_var[7]=(int16_t)debugAccelvar4; //  surface quality
    blackboxCurrent->log_var[8]=(int16_t)debugAccelvar5; // shutter
    blackboxCurrent->log_var[9]=(int16_t)debugAccelvar3; // Accel_correction Y
    blackboxCurrent->log_var[10]=(int16_t)getEstAltitude();
    blackboxCurrent->log_var[11]=(int16_t)RangingMeasurementData_L1.RangeMilliMeter; // Laser Altitude
    ...
    /*blackboxCurrent->log_var[0]=(int16_t)VelocityX;
	blackboxCurrent->log_var[1]=(int16_t)VelocityY;
	blackboxCurrent->log_var[2]=(int16_t)debugOpticFlowVar;
	blackboxCurrent->log_var[3]=(int16_t)debugOpticFlowVar;
	blackboxCurrent->log_var[4]=(int16_t)altholdDebug1;
	blackboxCurrent->log_var[5]=(int16_t)altholdDebug2;
	blackboxCurrent->log_var[6]=(int16_t)RangingMeasurementData_L1.RangeMilliMeter;
	blackboxCurrent->log_var[7]=(int16_t)debugOpticFlowVar4;
	blackboxCurrent->log_var[8]=(int16_t)debugOpticFlowVar5;
	blackboxCurrent->log_var[9]=(int16_t)heading;
	blackboxCurrent->log_var[10]=(int16_t)getEstAltitude();
	blackboxCurrent->log_var[11]=(int16_t)getEstVelocity();
	*/
    // Alt hold - vel control
    /*blackboxCurrent->log_var[0]=(int16_t)VelocityX;
	blackboxCurrent->log_var[1]=(int16_t)VelocityY;
	blackboxCurrent->log_var[2]=(int16_t)altholdDebug3;
	blackboxCurrent->log_var[3]=(int16_t)altholdDebug4;
	blackboxCurrent->log_var[4]=(int16_t)altholdDebug1;
	blackboxCurrent->log_var[5]=(int16_t)altholdDebug2;
	blackboxCurrent->log_var[6]=(int16_t)RangingMeasurementData_L1.RangeMilliMeter;
	blackboxCurrent->log_var[7]=(int16_t)debugOpticFlowVar4;
   ...
   // blackboxCurrent->rcCommand[3] = rcCommand[3];
    ...
    //Pass NED acceleration instead
    /**/
    blackboxCurrent->accSmooth[0] = lrintf(accel_ned.V.X);
    blackboxCurrent->accSmooth[1] = lrintf(accel_ned.V.Y);
    blackboxCurrent->accSmooth[2] = lrintf(accel_ned.V.Z);
    /**/
...
#ifdef BARO
    blackboxCurrent->BaroAlt = lrintf(BaroAlt);//(int32_t)getBaroPressure();//
#endif
