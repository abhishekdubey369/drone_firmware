...
uint8_t SPI_P::read(uint8_t register_address)
{
      ...
      //register_address &=  ~0x80u; //Not sure why this is necessary
...
void SPI_P::read(uint8_t register_address, int16_t length,uint8_t* buffer)
{
      ...
      register_address &=  ~0x80u;	//This is to ensure, we read the register.
    ...
    //    GPIO.write(Pin14, STATE_LOW);
        ...
        delayMicroseconds(15);	//min 10.5 uS

        spiTransfer(SPI2, buffer, NULL, length);

        delayMicroseconds(2);	//min 2 uS. Time between subsequent reads
...
//    DISABLE_SPI;
        ...
        register_address  |= 0x80u;	//This is to "write" the register
...
//	    GPIO.write(Pin14, STATE_HIGH);
	    ...
	    delayMicroseconds(15);	//min 10.5 uS between write commands
