...
int16_t debugAccelvar4=0;
int16_t debugAccelvar5=0;
...
#define FILTER_GAIN1 8
#define FILTER_GAIN2 20
...
{
    static int32_t accZoffset = 0, accXoffset = 0, accYoffset = 0;
    static int32_t accToffset = 0;
    float *tempVector;
   ...
   // dT = (float) deltaT * 1e-6f;
    ...
    rpy.angles.yaw = (float)0;

    //IMPORTANT change log
    //For now we convert to HBF rather than NED. Yaw estimation algo is not working properly
    ...
    if(print_timer==285){
		//print_timer = 0;
		Monitor.println("bZ ", accel_ned.V.Z);
		Monitor.println("bX ", accel_ned.V.X);
		Monitor.println("bY ", accel_ned.V.Y);
		Monitor.println(" ");
	}
    ...
    if(print_timer==285){
		print_timer = 0;
		//Monitor.println("aZ ", accel_ned.V.Z);
		//Monitor.println("aX ", accel_ned.V.X);
		//Monitor.println("aY ", accel_ned.V.Y);
		Monitor.println("Roll ", inclination.values.rollDeciDegrees);
		Monitor.println("Pitch ", inclination.values.pitchDeciDegrees);

		Monitor.println(" ");
	}

    /*debugAccADC[0]=(int16_t)accel_ned.V.X;
    debugAccADC[1]=(int16_t)accel_ned.V.Y;
    debugAccADC[2]=(int16_t)accel_ned.V.Z;*/


    if (imuRuntimeConfig->acc_unarmedcal == 1) {
        if (!ARMING_FLAG(ARMED) && accZoffsetCalCycle) {
            if(accZoffsetCalCycle==100)
            {
            	//Initialise filters for faster conversion
            	accXoffset = accel_ned.V.X * FILTER_GAIN2;
				accYoffset = accel_ned.V.Y * FILTER_GAIN2;
				accZoffset = accel_ned.V.Z * FILTER_GAIN2;
				accToffset = 0;

            }

            accToffset += accel_ned.V.X;


        	accZoffset -= accZoffset / FILTER_GAIN2;
            accZoffset += accel_ned.V.Z;


            accXoffset -= accXoffset / FILTER_GAIN2;
            accXoffset += accel_ned.V.X;
            //accXoffsetCalCycle--;

            accYoffset -= accYoffset / FILTER_GAIN2;
            accYoffset += accel_ned.V.Y;
            //accYoffsetCalCycle--;

            accZoffsetCalCycle--;
            // debug parameters
            debugAccelvar4 = accXoffset;
            debugAccelvar5 = accel_ned.V.X;
            //delay(10);
            //Monitor.print(" ", accZoffsetCalCycle);

            /*if(!accZoffsetCalCycle)
            {
            	Monitor.println(" ");
            	Monitor.println("Offset Z ", accZoffset / FILTER_GAIN2);
            	Monitor.println("Offset X ", accXoffset / FILTER_GAIN2);
            	Monitor.println("Offset Y ", accYoffset / FILTER_GAIN2);
            	Monitor.println("Offset T ", accToffset);
            }*/

        }
        accel_ned.V.Z -= accZoffset / FILTER_GAIN2;  // compensate for gravitation on z-axis
        //accel_ned.V.X -= accXoffset / FILTER_GAIN2;	//Not giving proper results
        //accel_ned.V.Y -= accYoffset / FILTER_GAIN2;  //Not giving proper results


        /*if(print_timer==285){
        	print_timer = 0;
        	Monitor.println("Z ", accel_ned.V.Z);
			Monitor.println("X ", accel_ned.V.X);
			Monitor.println("Y ", accel_ned.V.Y);
			Monitor.println(" ");
        }*/

        //Monitor.println("Test!!!! ",1);


        /*debugAccADC_trims[0]=accXoffset/8;
        debugAccADC_trims[1]=accYoffset/8;
        debugAccADC_trims[2]=accZoffset/8;*/
    ...
    accSum[Z] += lrintf(accel_ned.V.Z); //deadband doesnt help!

    //accSum[Z] += applyDeadband(lrintf(accel_ned.V.Z), accDeadband->z);

    /* Not sure why this was declared
...
//   float heading = calculate_heading();
   ...
   new_value = true;
   yaw_error = yaw_error_compass(); //This causes noise in del t
...
void imuUpdate(rollAndPitchTrims_t *accelerometerTrims)
{
    ...
    if (sensors(SENSOR_ACC)) {
        updateAccelerationReadings(accelerometerTrims); // Read new acc data and subtract trims
