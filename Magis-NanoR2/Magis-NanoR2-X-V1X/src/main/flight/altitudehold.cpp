...
//#include "flight/opticflow.h" //Only for debugging
...
int QUEUE_MAX_LENGTH = 7; //change log original 15 = 150ms delay For
#else
...
#if defined(LASER_ALT) || defined(LASER_ALTITUDE)
float _time_constant_z=1.2f;
#else
...
int16_t altholdDebug = 0;
int16_t altholdDebug1 = 0;
int16_t altholdDebug2 = 0;
int16_t altholdDebug3 = 0;
int16_t altholdDebug4 = 0;
...
#if defined(BARO) || defined(SONAR) || defined(LASER_ALT) || defined(LASER_ALTITUDE)
...
//        initialThrottleHold = THROTTLE_BALANCE_VALUE;
...
int32_t calculateAltHoldThrottleAdjustment(int32_t velocity_z, float accZ_tmp, float accZ_old)
{
    ...
    if (!velocityControl) {
        ...
        error = constrain(AltHold - EstAlt, -200, 200);
		#ifndef LASER_ALTITUDE
        ...
        setVel = constrain((pidProfile->P8[PIDALT] * error / 128), -120, +120); // limit velocity to +/- 3 m/s

    } else {

        setVel = setVelocity;
    }
    altholdDebug2 = setVel;
  ...
  //  altholdDebug1 = setVel;
    ...
    result = constrain((pidProfile->P8[PIDVEL] * error / 32), -200, +200);

    //debugOpticFlowVar2 = (int16_t)result;

    // velControlDebug[0] = result;

    // I
    if (ARMING_FLAG(ARMED)) {
    	// error_I = constrain(error,-20,20); // Modified on 9/10/21
        //errorVelocityI += (pidProfile->I8[PIDVEL] * error_I);
    ...
    //debugOpticFlowVar3 = (int16_t)(errorVelocityI / 8192);
    ...
    //debugOpticFlowVar4 = (int16_t)(constrain(pidProfile->D8[PIDVEL] * (accZ_tmp + accZ_old) / 512, -150, 150));

    //velControlDebug[2] = constrain(pidProfile->D8[PIDVEL] * (accZ_tmp + accZ_old) / 512, -150, 150);
    //debugOpticFlowVar = (int16_t)AltHold;
    //debugOpticFlowVar1 = (int16_t)setVel;

    altholdDebug3 = (int16_t)(errorVelocityI / 8192);
   // altholdDebug4 =  constrain(pidProfile->D8[PIDVEL] * (accZ_tmp + accZ_old) / 512, -150, 150);
...
void apmCalculateEstimatedAltitude(uint32_t currentTime)
{
    ...
    if (dTime < UPDATE_FREQUENCY)	//10ms
        return;
...
#if defined(BARO) && !(defined(LASER_ALT)) && !(defined(LASER_ALTITUDE))
    checkBaro(); // check if new baro readings have arrived and use them to correct vertical accelerometer offsets.
#elif defined(LASER_ALT)
    checkReading();
#elif defined(LASER_ALTITUDE)
    checkLaser();
#endif
	...
	accZ_tmp = accel_ef_z + _accel_correction_hbf_z/accVelScale;
	altholdDebug = (int16_t)accel_ef_z;
	accel_ef_z = accel_ef_z * accVelScale;
	accel_ef_z = constrainf(accel_ef_z, -800, 800);

	accel_ef_z = accel_ef_z + _accel_correction_hbf_z; //Correction is done here

    // calculate velocity increase adding new acceleration from accelerometers
    float velocity_increase_z;
    velocity_increase_z = (accel_ef_z) * dt;

    // calculate new estimate of position
    _position_base_z += (_velocity_z + velocity_increase_z * 0.5) * dt;
    altholdDebug4 = _position_base_z;
    ...
    //debugOpticFlowVar5 = (int16_t)_accel_correction_hbf_z*10;
...
void checkReading()
{
    ...
    if(isTofDataNew() && (!isOutofRange())) {
        ...
        if(tilt < 25)
        ToF_Height *= cos_approx(tilt); // AdityaLakshmi // warp angles to -pi to pi
    }
    //Fusion
    ...
    if(isTofDataNew_L1() && (!isOutofRange_L1())) {
		...
		tilt = degreesToRadians(calculateTiltAngle(&inclination)/10);
		if(tilt < 25)
		ToF_Height *= cos_approx(tilt); // consider roll and pitch together
        }
    //Fusion
	if (ToF_Height > 0 && ToF_Height < 350 ) {
		baro_offset = Baro_filtered-EstAlt;
		correctedWithTof(ToF_Height);   // *** This needs to happen when the new data arrives
	} /* else
	 //{ Baro_Height -= baro_offset;
	 if (ToF_Height >= 120  && ToF_Height <= 200) {
	 //tofTransition = (200 - ToF_Height) / 100.0f;
...
// AdityaLakshmi
// Laser estimate
#ifdef LASER_ALTITUDE
void checkLaser()
{
	// laser latency correction
	float tilt = 0;
    ...
    if(isTofDataNew_L1() && (!isOutofRange_L1())) {
		...
		ToF_Height = (float)NewSensorRange_L1/10.0f;
		isTofDataNewflag_L1 = false;	//Change for L1x
...
void correctedWithTof(float ToF_Height) {
    ...
    _position_error_z = ToF_Height - (_position_base_z + _position_correction_z); //Change made, needs to be tested!!
    if(_time_constant_z!=1.2f) {
        _time_constant_z = 1.2;
        updateGains();
    }
}
#endif
